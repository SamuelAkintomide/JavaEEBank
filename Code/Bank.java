...
class Access{
    ...
    int transfer = 101;//1 above number of accounts that can be used
    int number;
    int count = 0;
    String accStatus;
    String name;
    ...
    HashMap<Integer, String> transacNum = new HashMap<>(); 
    HashMap<Integer, Double> transacPos = new HashMap<>();
    ...
    public void recentTransactions() {
        if(transacPos.size() > 1){
            System.out.println("\nCurrent Balance: $"+balance);
            System.out.println("\nLast "+transacPos.size()+" transactions order of size:");
        } else if(transacPos.size() == 1){
            System.out.println("\nCurrent Balance: $"+balance);
            System.out.println("\nLast transaction:"); 
        } else if(transacPos.size() == 0){
            System.out.println("\nNo transaction have been made\n");
        }       
        HashMap<Integer, Double> temp = 
                (HashMap<Integer, Double>) transacPos.clone();
        int pos = 0;
        ...
        for(Integer x:posArr){
            if(x != null){
                System.out.println(transacNum.get(x));
            }
        }  
    }
    ...
    public void changeHash(){ 
        ...
        transacNum.put(count, transactions);
        transacPos.put(count, change);
        count ++;
        if(transacNum.size() == 7){ //Assures only last 6 transactions displayed 
            transacNum.remove(count-7);
            transacPos.remove(count-7);
        }
    }
...
public class Bank {
    public static void main(String[] args) {
        Scanner input = new Scanner(System.in);
        boolean user = true;//keeps user in the interface unless loop broken
        boolean found = false;
        int numberOfAccounts = 100;//number of accounts
        int arr[] = new int[numberOfAccounts];
        Access C[] = new Access[numberOfAccounts];//Access incapsulation
        int n = 0; 
            ...
            while(picked == false){
                ...
                if(status != null){
                    if ("returning".equals(status)) { //Created Accounts
                        ...
                        picked = true;
                    } else if ("delete".equals(status)) {//Delete account
                        System.out.println("Enter account number?");
